...
`android:device` 会检查 `ANDROID_HOME` / `adb`、选择已授权真机、编译 Debug
Development Build，并做 Metro `8081` 端口 reverse。可选传入序列号：

```bash
npm run android:device -- emulator-5554
```

首次运行会生成被 `.gitignore` 忽略的 `android` 原生目录。当前开发机尚未安装
Android 工具链时，脚本会在 Gradle 开始前失败并说明缺了什么，不要用 Expo Go 绕过。
...
iOS 与 Android 已完成真实 BLE 运动控制、正式页面和设备设置写入。三客户端并发已通过。
BLE/Wi-Fi 超距自动回退异常矩阵以及双平台内测发布仍是开放门禁。

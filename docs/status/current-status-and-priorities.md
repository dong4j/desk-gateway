...
| P1 | 自定义档位与 B12 恢复 | 自定义档位、跨端同步、故障建议和 REST/BLE 控制盒重置均已实现；无位移看门狗已关闭，避免误 STOP | 真桌验证档位增删改、重启持久化、8 秒重置、提前 STOP 和边界无误报 |
...
1. Matter、Siri / App Intent 和米家/华为生态接入。
2. OTA 固件升级。
3. Loctek、Jiecang 等其他桌型 Driver。
4. 原厂档位 2 / 3 协议逆向与实现。
5. 原厂“上+下约 5 秒重置”键码抓包与原按键语义复现；当前 `DR=0x7F` 等价重置入口已经实现，但不等同于完成原按键协议逆向。
6. BLE OLED / 独立旋钮参考配件。
7. 原厂面板 / 控制盒继电器旁路：方案已选定（DR21A01 5V ×2 + MTS-102），文档见
   [硬件方案](../hardware/panel-bypass-relay.md) 与
   [实施计划](../history/plans/2026-08-20-panel-bypass-relay.md)。
   真机未接线、固件未实现，不得记为已完成。

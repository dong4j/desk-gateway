...
| `11` | PRESET_1 | 闭环前往设备配置的档位 1（默认 56 cm） |
...
13. 写 `02 06 30 02` 将档位 1 设为 560 mm，写 `02 07 66 03` 将档位 4 设为
    870 mm；Web 与 App 均应自动显示 56 / 87 cm，重启固件后仍保持。违反
    `560 <= 档位1 < 档位4 <= 940` 的写入必须失败。

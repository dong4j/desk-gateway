...
| `11` | PRESET_1 | 闭环前往设备配置的档位 1（默认 55 cm） |
| `14` | PRESET_4 | 闭环前往设备配置的档位 4（默认 87 cm） |
...
- 下降仍有 `15s` 总运动超时；上升还会被 TOF400C 的 94 cm 上限以及 80 cm 以下的右侧障碍策略停止。
...
11. 读取 Config `7f4e0004-...`，应得到 10 字节 v3 快照；写 `03 01 01 00` 开启童锁，
    再读应看到 bit0=1；写 `03 01 00 00` 关闭童锁。写 `03 08 26 02` 将最低档位设为
    550 mm，确认快照末两字节为 `26 02`。
12. 写 `02 05 AC 03` 将安全上限设为 940 mm，Config Read 和 State Read 均应回读
    `AC 03`；无效范围必须 Write 失败，并验证上升到 940 mm 时自动停止。
13. 为验证非默认值写入，写 `03 06 30 02` 将档位 1 临时设为 560 mm，写
    `03 07 66 03` 将档位 4 设为 870 mm；Web 与 App 均应自动显示 56 / 87 cm，
    重启固件后仍保持。违反 `min_height_mm <= 档位1 < 档位4 <= 940` 的写入必须失败；
    验收结束后写 `03 06 26 02` 恢复当前默认坐姿 550 mm。

...
| 上+下同时长按 | **重置**；`DR=0x7F`，成功抓包约持续 **7.52 s** |
...
| 规则 | 做法 |
|---|---|
| 不完整帧 | 未建立可信高度前，缺地址 / 缺数据 / 无 ACK → 丢弃 |
| 非 shaped 四元组 | digit 缺一即丢；优先要求四地址齐全 |
| 非 `DW=0x01` 的键写 | 不计入协议 |
| 非 `0x24`/`0x34–0x37` 地址 | 运动段多视为 EMI |
| **优先静态保存包** | `preset_*_save.sr` 段码 100% 干净，作字库与回归金标准 |
| 运动包 | 完整帧占比低（约 7–17%）；完整帧仍按 20 ms 严格组帧。上升和下降共用分代缓存恢复碎片，解出一代后清空接收掩码，防止下一代单个数字与两位旧数字拼成假高度；方向与最大速度过滤只作用于完整候选。曾尝试的上升专用「寄存器镜像 + 100 ms settle」会让两种方向进入不同状态机，已撤销 |
...
| digit：基线前缺 `0x34` 的三写 | 不更新高度 |
| digit：基线后仅 `0x36` 更新 | 更新锁存的个位候选；仍须通过方向与速度过滤 |
...
| 上+下同时长按 / 重置 | `0x01` | **`0x7F`** | `0111_1111` | `captures/重置.sr` 实测主导码 |
...
`captures/重置.sr` 记录了原厂面板同时长按上升和下降、成功清除 B12 的完整过程。采样率
为 **12 MHz**，总计 **100 M** 点（约 8.333 s）。键轮询中：

- `DR=0x7F` 共出现 **1303** 次，从约 **0.4389 s** 持续到 **7.9551 s**，有效区间约 **7.516 s**；
- `DR=0x3F` 仅出现 **63** 次，属于按键叠加或释放过程中的过渡值，不作为注入码；
- 前后出现的 `0x0F` 同样不替代空闲码，固件结束时仍必须明确恢复 `DR=0x2E`。

因此产品实现固定输出 `0x7F` **8 秒**，随后由固件定时器主动 STOP 并恢复 `0x2E`。
浏览器断开不能延长该状态，任何入口的 STOP 都可以提前中断。该实现仍需烧录后用真实 B12
故障完成一次硬件验收。
...
面板 = **TM1650 Slave**；控制盒 = **Master**。协议研究已经证明同一总线上存在 `0x24` 键通道
和 `0x34–0x37` digit 显示通道，段码解码器也通过了离线金标准向量。但“知道协议”不等于
“软件模拟实现满足实时性”：统一软件多地址 Slave 在持续升高时记录到大量 `0x24` 发送事务中断，
控制盒间歇读不到完整 `0x47`，桌子因此断续。

产品固件现已回到已知稳定提交 `3269faa` 的原则：ESP32-S3 硬件 I²C Slave 只应答 `0x24`，
不 ACK、不解析 `0x34–0x37`。控制盒 digit 不再作为产品高度。产品高度由独立 TOF400C 提供，
TOF050C 只做低位右侧保护。完整经过见
[`hardware/i2c-restoration.md`](../hardware/i2c-restoration.md)，传感器策略见
[`tof-safety.md`](./tof-safety.md)。
...
| **已知键码注入** | 当前产品路径至少支持：`idle=0x2E`、`UP=0x47`、`DOWN=0x4F`。档位码作为协议研究结果保留。 |
| **稳定停止** | 松手、显式 STOP、童锁或来源权限拒绝时恢复 `0x2E`；不得依赖高度帧决定手动升降是否继续。 |
...
| 控制盒 digit 高度解析 | 软件多地址方案已退出产品路径；研究代码和离线向量保留，等待独立硬件实验需要 |
| Preset 闭环 | 已知键码保留，但没有可靠高度源时不对外声明支持；独立 ToF 验收后再恢复 |
...
- [x] 上+下同时长按重置：`captures/重置.sr` 主导 `DR=0x7F`，持续约 7.52 s（§9.7）
...
4. Web、REST、BLE 和 App 在独立 ToF 接入前必须显示高度未知；档位闭环与上升限高见 [`tof-safety.md`](./tof-safety.md)。
...
| `captures/重置.sr` | 同时按住上升+下降，成功清除 B12；`DR=0x7F` | **已完成（§9.7）** |

抓包注意：

1. 从静置开始录，覆盖「双手按下 → 维持 ≥5s → 松开 → 再静置」。  
2. 记录面板显示变化（是否清高度、闪码、进设置等）。  
3. 重置注入只使用已验证的 `0x7F`；`0x3F` 等过渡值不进入产品实现。  
4. 真机验收必须确认定时结束和手动 STOP 后都恢复 `0x2E`。
...
| **1.7** | **2026-08-11** | 同步旋钮 jog 时序：700 ms 启动窗口、500 ms 运动租约，反向首刻度先 STOP |
| **1.8** | **2026-08-12** | 全局复盘软件多地址发送事务中断；产品默认恢复硬件 I²C `@0x24`，控制盒 digit 高度与档位闭环停用，等待 TOF200C |
| **1.9** | **2026-08-14** | 解出 B12 重置抓包：`DR=0x7F` 持续约 7.52 s；Web 实现采用 8 s 固件定时输出并保证 STOP 恢复 `0x2E` |
...
| **上+下同时长按 → 重置** | **`0x7F`** | 固件固定 **8 s** | 抓包有效区间约 7.52 s；结束必须回 `0x2E` |

旋钮 jog 映射示例：

```text
首个旋钮刻度           → 只进入待命，保持 0x2E
700 ms 内第二个同向刻度 → 顺时针回 0x47，逆时针回 0x4F
连续同向刻度           → 每次刷新 500 ms 运动租约
停止旋转               → 最后一个刻度后约 500 ms 回 0x2E
运动中首个反向刻度     → 立即回 0x2E；第二个反向刻度才启动
单击「坐姿档」   → 持续回 0x17 一段时间
长按「保存坐姿」 → 持续回 0x57 ≥ 4 秒，再回 0x2E
```

Web 和手机 App 的按住升降是另一条 HOLD 路径，具有明确的按下、续期和
松手 STOP 事件，不使用上述旋钮 jog 窗口。
...
| 写 digit `0x34–0x37`（`0x68–0x6E`） | 产品固件不应答；只在独立协议实验中由软件多地址实现接收 |

### 18.4 历史实验：从 digit 写读取高度（产品固件已停用）

高度不是 ESP32「发送」出来的，而是控制盒写给数码管的段码。以下步骤只用于离线分析或
独立实验分支，不是当前产品固件的运行逻辑：
...
4. 被动 GPIO 边沿监听和统一软件多地址 Slave 均已退出产品路径。
5. 产品高度由独立 TOF400C 提供，不再解析控制盒 digit。

### 18.5 安全与时序（必须遵守）

1. **停止优先**：异常、断连、任务切换时，先把 `DR` 置 **`0x2E`**。  
2. **升/降 = 按住**：回 `0x47`/`0x4F` 就会持续动；上升由松手或显式 STOP 回 `0x2E`，下降保留 15 秒连续运动超时。最高安全高度拦截当前停用。

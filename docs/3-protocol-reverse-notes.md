...
看门狗从最后可信锚点按抓包实测速度加余量（`27 mm/s`）提前制动，不等待下一帧。缓存候选仅在完整组装并通过方向过滤时成为安全锚点，锚点时间取候选提交时刻，不再错误回退到最旧数字碎片的时间。桌子最低位已真机确认为 `64 cm`；
...
4. 上升与下降恢复为同一套「严格完整帧 + 分代缓存」路径，不再维护上升专用寄存器镜像。新增英制 `37.5 → 38.5 → 39.0 → 39.5 → 40.0 in` 稀疏反馈链路回归，覆盖段码解码、方向过滤、预测上限和档位停止；主机测试与固件构建通过后，仍需真机确认连续性和上限保护。

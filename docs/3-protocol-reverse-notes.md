...
| 运动包 | 完整帧占比低（约 7–17%）；完整帧仍按 20 ms 严格组帧。可信完整帧建立基线后，上升路径按真实 TM1650 的寄存器锁存行为接纳单地址更新，再用方向与最大速度过滤；下降路径保持原分代缓存逻辑 |
...
4. 上升碎片恢复已改为 TM1650 寄存器镜像模型；当前仅完成抓包回放、主机测试与固件构建，仍需真机确认连续性和上限保护。

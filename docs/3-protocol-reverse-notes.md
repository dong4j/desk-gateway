...
| 版本 | **1.2.1** |
| 日期 | 2026-08-06 |
| 状态 | 进行中（**面板 IC = TM1650**；按键在控制地址读；**高度在 digit 段码写**） |
| 对应阶段 | Phase 0 |
...
**解码器必须两种都接受**：以「完整地址 + 数据 + ACK/NACK 语义」为准，不以「必须 Sr」为硬条件。
...
厂家线从壳体引出四色线焊在板边：黑 / 红 / 白 / 绿，对应电源与 CLK/DAT（具体色序以实现时丝印与万用表为准；逻辑上就是 `GND` / `3.3V` / `CLK` / `DAT`）。
...
形态 A（常见，Repeated Start）：
  S  AW:24  ACK  DW:01  ACK  Sr  AR:24  ACK  DR:xx  NACK  P

形态 B（分开 START–STOP，亦须接受）：
  S  AW:24  ACK  DW:01  ACK  P
  S  AR:24  ACK  DR:xx  NACK  P
```

判定规则（确定性）：

1. 写半段：`addr7==0x24` 且 `rw==W`，恰好 1 字节数据，且 **`DW == 0x01`**（否则丢弃 / 标噪声）。
2. 读半段：`addr7==0x24` 且 `rw==R`，恰好 1 字节数据；Master 以 **NACK** 结束读（ACK 后继续读则异常）。
...
1. 在短时间窗内（典型整帧 ~7 ms；实现可用 ≤20 ms 或「连续 digit 写直至收齐 4 个不同地址」）收集：
   - `dig[1] ← 写往 0x34 的 seg`
   - `dig[2] ← 写往 0x35 的 seg`
   - `dig[3] ← 写往 0x36 的 seg`
   - `dig[4] ← 写往 0x37 的 seg`
2. **总线写序**多为 `0x36 → 0x35 → 0x34 → 0x37`（DIG3→2→1→4）。收集时 **按地址归档**，不要把「第 1 个到达的字节」当百位。
...
2. **同一数字的 DP 变体**：高位 bit7 常翻转（如 `0x5F/0xDF`、`0xC5/0x45`），扩展字库时把差值为 `0x80` 的对子视为同一数字候选。
3. **标准共阴表 `0x3F/0x06/…` 不可用**（popcount 与观测不兼容）；禁止 bit 重排硬套。
4. **扩展未知数字（3/5/7/8/9）的正确做法**：
   - 在 **静置**、已知面板显示高度下抓短包；
   - 取 shaped 四元组，用已知位（例：已标定的 0/1/2/4/6）锚定，反推未知位段码；
   - 同一数字至少 2 个不同高度交叉验证后再写入表。
5. 未知段码：**不更新 `height_cm`**，可打日志 `raw=(d1,d2,d3,d4)`。
...
| 运动包 | 完整帧占比低（约 7–17%）；多数表决 / 时间滤波；锚点帧可信 |
...
|---|---|---|---|
| `DW` | `0x01` | **`0x01`** | **`0x01`** |
| `DR` | `0x2E` | **`0x47`（全程，约 109 次）** | **`0x4F`（全程，约 108 次）** |

要点：

1. 事务骨架未变：`AW:24` → `DW:01` → `AR:24` → `DR:??` → NACK → Stop。
2. 按键信息落在 `DR`。
3. 本两文件 **无 press / release 边沿**（全程已是按住态）。
4. DOWN 抓包约 383–391 ms 偶见非 `0x24` 地址毛刺，更像采样/同步噪声，**不以之为协议字节**。
...
尚未用锚点锁死 3/5/7/8/9；`0x46` 在 goto 下降段频繁作十位，疑似 **7**（待专次标定）。标准 TM1650 字库 **不能** 经简单 bit 重排得到上表（popcount 不一致），可能是板级段脚映射 / 采样偏置 / 厂定字模。网关应用 **经验表**，或在静置高 SNR 下再标定全集。
...
面板 = **TM1650 Slave**；控制盒 = **Master**。对后续固件（预告，未实现）划分如下。
...
| **旁路嗅探高度**（若 Phase1 要上报高度） | 听 digit 写 `@0x34–0x37`，按 §8 组装 `height_cm`（整数 cm）；不完整帧丢弃。 |
...
| 完整 0–9 段码字库 | 已知 0/1/2/4/6 已够解 64/102/121；3/5/7/8/9 待静置标定 |
| Preset2 / Preset3 | **键码未知**；等抓包后再实现 |
| Goto 开头的 `0x07`/`0x0F` | 语义未证实；首版可忽略 |
| 运动段高度的精细跟踪 | 静态帧已够；运动需更强滤波 |
...
2. `S`（Repeated Start）→ `Address read: 24` → `AC` → `Data read: 2E` → `NACK` → `P`
...
- [x] 长按上升（按住态）：`DR` 由 `0x2E` → **`0x47`**；`DW` 仍为 `0x01`（见 `captures/up_hold.sr`）
- [x] 长按下降（按住态）：`DR` 由 `0x2E` → **`0x4F`**；`DW` 仍为 `0x01`（见 `captures/down_hold.sr`）
...
- [ ] `DW:01` 控制字节语义细化（亮度位）；静置多高度标定 0–9 段码全集
- [ ] 专次静置抓包标定段码 3/5/7/8/9（无电机）
...
| 文件名 | 操作 | 优先级 |
|---|---|---|
| `height_idle_NN.sr` | **静置** 时面板显示 NN（建议 60–70、80、90、100、110、120 各一段短窗） | **高** |
| — | 目的：无电机噪声下锁死 0–9 经验段码（补 3/5/7/8/9） | |
...
|---|---|
| GND | GND |
| CLK | 任意支持 I²C 的 SCL GPIO |
| DAT | 对应 SDA GPIO |
| 3.3V | **不要**用桌子 3.3V 给 ESP32 供电（USB 独立供电）；**必须共地** |
...
| `AR:24` 读 1 字节 | ACK 地址后，把当前 `DR` 驱动到 SDA；Master 以 NACK 结束读 |
| 写 digit `0x34–0x37`（`0x68–0x6E`） | **旁路读高度**：只听；完全替面板且要亮数码管时才自己处理显示 |

### 18.4 读高度（旁路嗅探，不是改 `DR`）

高度不是 ESP32「发送」出来的，而是控制盒写给数码管的段码。旁路时：

1. 监听写往 `0x68 / 0x6A / 0x6C / 0x6E`（7-bit：`0x34–0x37`）的 1 字节段码  
2. 收齐 DIG1/2/3（+DIG4）后查经验字库 → `height_cm`  
3. 金标准：`00 DB C5 C5` → **64 cm**；`44 5F 9E 9E` → **102 cm**（字库见 §8.5 / §10.3）
...
ESP32 @ I²C Slave 0x24
  ...
  读高度   → 旁路听 0x68/6A/6C/6E 段码（不是改 DR）
```

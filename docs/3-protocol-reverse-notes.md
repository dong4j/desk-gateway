...
| 版本 | **1.5** |
| 日期 | 2026-08-10 |
| 状态 | 进行中（**面板 IC = TM1650**；GPIO 高度嗅探真机 NO-GO，稳定版已关闭） |
| 对应阶段 | Phase 0 |
...
段码解码器已通过离线金标准向量，但“硬件 `0x24` Slave + GPIO 边沿嗅探”在真机上干扰升降且没有
收到高度 data，已通过实验开关默认关闭；真实高度改走统一软件多地址 Slave。
...
| **多地址接收高度**（若 Phase1 要上报高度） | 同一软件 I²C 状态机应同时 ACK `0x24` 和 digit `0x34–0x37`；收到 digit data 后按 §8 组装 `height_cm`。纯 GPIO 监听已真机否决。 |
...
**A. 实现软件多地址 I²C Slave（高优先）**

1. 稳定版保持 `CONFIG_DESK_YOURDESK_HEIGHT_SNIFFER_EXPERIMENTAL=n`，先保证 `0x24` 升降不回归。
2. 新状态机统一处理 `0x24` 键读写和 `0x34–0x37` digit 写，并对五个地址正确 ACK。
3. 先用逻辑分析仪验证 ACK/data，再接入现有段码解码器；不得重新启用高频 GPIO 被动嗅探冒充完成。
...
4. 真机已证明当前 GPIO 边沿监听会影响 `0x24` 控桌且拿不到有效高度，稳定版默认关闭。
5. 下一实现必须统一 ACK `0x24` 与 `0x34–0x37`；SIM 只能带明确徽章用于演示，不能冒充真实高度。

...
| 运动包 | 完整帧占比低（约 7–17%）；完整帧仍按 20 ms 严格组帧。可信完整帧建立基线后，上升路径先将单地址变化写入待定寄存器，连续 **100 ms** 无新数字变化后才解码（重复刷新相同段码不续期），再用方向与最大速度过滤；异常前跳回滚到已确认寄存器，避免把 `27.5 → 28.0` 的瞬态 `57.5` / `28.5` 当真。下降路径保持原分代缓存逻辑 |
...
4. 上升碎片恢复采用「待定 / 已确认」两份 TM1650 寄存器镜像，并等待 100 ms 安静窗口后才发布；已加入 `69.9 cm → 146.1 cm → 72.4 cm → 71.1 cm` 真机故障序列回归。当前仅完成抓包回放、主机测试与固件构建，仍需真机确认连续性和上限保护。

...
运动被错误打断。当前保留最后可信高度，不因普通停止或无效下降清空；上升限高由独立 `100ms`
...
启用 Wi-Fi 与 NimBLE 后，控制盒侧软件 I²C GPIO ISR 必须与无线协议栈隔离：ISR 服务在 Core 1
安装，digit 解码和无线任务留在 Core 0；digit 入队不会在相邻约 `52 us` 的 SCL 边沿之间立即
触发任务切换。固件累计已完成的 `0x24` DR 读取，并只在运动期间检测到至少 `20 ms` 的轮询
缺口时告警，不对约 `3.7 ms` 的每次正常轮询输出日志。

GPIO 中断不会同时锁存 DAT 跳变发生时的 CLK 电平，因此延迟送达的普通数据边沿不能直接作为
START/STOP 依据。当前代理按实测的单字节事务自行收尾：完成 `0x24`/`0x34–0x37` 写数据 ACK
或完成 `0x24` 单字节 DR 读取后，立即回到地址等待态；只有确实处于事务边界时才接受 START，
帧中间的延迟 DAT 边沿和 STOP 不再重置活动事务。这样既兼容实测的 STOP 分隔，也兼容写后
Repeated START，并避免无线任务活跃时偶发破坏键轮询。

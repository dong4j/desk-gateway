...
运动被错误打断。当前保留最后可信高度，不因普通停止或无效下降清空；上升限高只接受已完整组装、
通过方向/速度过滤的控制盒高度，达到配置上限的提前停止余量后回 `0x2E`。不再按陈旧高度和经过
时间外推桌面位置，避免高度通道暂时缺帧时在中途误触发上限锁。桌子最低位已真机确认为 `64 cm`；
高度只允许由控制盒完整 digit 帧更新，不再提供人工 `64 cm` 覆盖。每次新运动的首个完整高度帧
自动成为重新同步基准，后续帧再执行方向和速度过滤。高度触顶只锁住继续上升，不清空主机高度；
下降收到安全区新帧后自动解除该锁。
...
| 运动段高度的精细跟踪 | 已加入严格帧边界、方向/速度过滤与反馈高度上限处理；缺帧时间本身不会触发误停，仍需真机覆盖全行程验收 |
...
4. 上升与下降恢复为同一套「严格完整帧 + 分代缓存」路径，不再维护上升专用寄存器镜像。新增英制 `37.5 → 38.5 → 39.0 → 39.5 → 40.0 in` 稀疏反馈链路回归，覆盖段码解码、方向过滤、反馈高度上限和档位停止；软件 I²C 状态机拒绝活动字节期间延迟到达的伪 START/STOP，主机测试通过后仍需真机确认连续性和上限保护。
...
2. **升/降 = 按住**：回 `0x47`/`0x4F` 就会持续动；上升由松手、显式 STOP 或最高安全高度回 `0x2E`，下降保留 15 秒连续运动超时。

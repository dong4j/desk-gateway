...
启用 Wi-Fi 与 NimBLE 后，控制盒侧软件 I²C 不能继续依赖主 CPU 的逐边沿 GPIO ISR：即使把 ISR
固定到另一个 Core，无线栈临界区仍可能造成数百毫秒级边沿缺口。当前固件把统一多地址 Slave
状态机移到 ESP32-S3 的 `17.5 MHz` ULP RISC-V 核心，独立轮询 GPIO4/5；主 CPU 只通过 RTC
Slow Memory 邮箱更新 `DR`，并从共享环形缓冲区取出 digit 事件。该结构不改变现有接线，也不对
控制盒约每 `3.7 ms` 的正常 `0x24` 轮询逐次输出日志。运动诊断只汇总 ULP 事务计数、环形缓冲区
丢帧数和 ULP 当前生效的 `DR`，用于区分入口命令、总线应答和高度解析问题。

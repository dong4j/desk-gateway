...
> **实现状态（2026-08-10）**：Phase 1 主固件已通过 ESP-IDF 6.0.2 编译；补回两只
> `2 kΩ` 总线上拉后，Web 按住升/降及松手停止已通过真机验证。统一软件多地址 Slave
> 也已通过升降与真实高度 `64–80 cm` 跟踪。档位 1/4、超时与重启安全、童锁状态流仍待
> 验收，因此尚不能视为 Phase 1 完成。

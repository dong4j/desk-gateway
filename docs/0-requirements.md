...
4. **优先级**：童锁 OFF 时原厂面板按键优先于无线；童锁 ON 时面板失效，网关侧仍可控。
5. **可扩展**：新增厂商 ≈ 新增 Driver；连通与生态层复用（含 Matter；米家/华为原生见生态调研）。
6. **童锁**：开启后原厂控制面板不能控桌。
7. **国内生态**：规划接入小米米家、华为智慧生活（适用性）；路径见架构生态调研。
...
| WiFi + 本地 Web（REST/SSE）+ 简单认证 | **必做** | 必做 |
...
| P1-F09 | WiFi STA + Web：简单密码登录、REST 控桌、SSE（或短轮询）推状态 |
| P1-F10 | Web UI：品牌、升降桌示意图、升/降时示意图实时动画 |
| P1-F11 | 童锁：可开关、进 status/SSE；语义为「原厂面板不可控」（Phase1 预埋；Phase2 真屏蔽） |
...
| G-S04 | 状态变化可推送（Phase 1：SSE 优先，失败则短轮询；未来可加 WebSocket / BLE notify） | P0 |
...
| G-A04 | **童锁**：开启后原厂控制面板不能控桌；Web/UART/BLE 仍可控制并关闭童锁；状态持久化 | P0 |
| G-A05 | 童锁开启时（Phase 2）：面板运动意图不转发/不生效；面板不得打断网关注入运动；急停仍以网关侧为准 | P0 |
| G-A06 | 仲裁优先级：急停 > 童锁屏蔽面板 >（童锁关）面板优先无线 > 无线 | P0 |
...
| G-W02 | 提供 REST：auth / up / down / stop / preset / child-lock / status | P0 |
| G-W03 | 提供 SSE（或短轮询）状态流，驱动 UI 升降动效（含 `child_lock`） | P0 |
...
| G-W07 | BLE 未绑定设备默认不可下发运动指令（防邻居乱控）；已绑定外设在童锁 ON 时仍可控制（与 Web 同级） | P1 |
...
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/desk/up` | 开始或点动上升 |
| POST | `/desk/down` | 开始或点动下降 |
| POST | `/desk/stop` | 立即停止 |
| POST | `/desk/preset/{n}` | 前往档位 n |
| POST | `/desk/preset/{n}/save` | 保存档位 n（若支持） |
| GET | `/desk/status` | 高度、运动状态、面板占用等 |
...
| 仲裁 | 童锁 OFF：原厂面板优先无线；童锁 ON：屏蔽面板，网关仍可控 | 2026-08-06 |

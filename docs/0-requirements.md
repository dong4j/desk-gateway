...
> **实现状态（2026-08-12）**：Web 基础控制、真实高度、档位 1/4、最高安全高度配置持久化、
> 全局童锁、BLE 核心控制和 iPhone App 已通过真机核心路径。异常停止矩阵、Android
> 真机以及双 RJ45 原厂面板透传仍待完成；唯一汇总状态以
> [当前状态与任务优先级](./5-current-status-and-priorities.md) 为准。
> 为优先恢复连续升降，最高安全高度的运动拦截当前临时停用，保存值不参与控制。

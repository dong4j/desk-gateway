...
| 优先级 | 项目 | 当前缺口 | 完成条件 |
|---|---|---|---|
| P0 | 连续升降恢复 | 共用软件 I²C 以 `67d14bb` 为基线，并仅保护 `0x24` 的 `TX_DATA / TX_MASTER_ACK` 发送阶段；高度限制不再介入运动，仍待真机分别验证按住升/降和档位全程连续 | `0x47`/`0x4F` 持续到显式 STOP，`key_tx` 完成数与开始数基本一致且 `abort=0`，中途无非预期停顿 |
| P0 | 异常停止安全矩阵 | 下降 15 秒超时已有真机日志；上升依赖显式停止，断连、重启、掉电、网络中断和 App 被杀尚未集中复验 | 按清单逐项留存串口与客户端结果 |
| P2 | 重新启用最高安全高度 | 配置仍持久化和同步，但运动拦截已临时停用 | 连续升降稳定后，用独立、可证明不干扰总线的方案重新设计和真机验收 |

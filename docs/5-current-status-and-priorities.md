...
- [x] TOF050C / TOF400C 已接入独立 I²C，总线数据可在 Web 和板载 OLED 实时显示。
...
| P2 | TOF400C 真实高度与安全上限 | 驱动、Web/OLED 显示已完成；安装偏移、全行程和不同地面材质尚未验收 | 64–129 cm 全行程稳定并完成校准后，再讨论档位闭环和最高安全高度 |
...
1. **双 ToF 运动安全输入：** TOF050C + TOF400C 的采集和显示已完成，校准及全行程验收前不参与运动；
   见 [双 ToF 接入方案](./4-tof-distance-sensor-plan.md)。
...
3. 完成 TOF400C 安装偏移、全行程和地面材质验收，再评估档位闭环与最高安全高度。

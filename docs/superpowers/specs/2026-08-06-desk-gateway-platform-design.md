...
5. **童锁**：开启后 **原厂控制面板失效**（无法控桌）；Web / 串口 / **BLE 外设** 等网关侧控制仍可用（含解锁）。
...
7. 童锁 ON 时屏蔽面板（Phase 2）；**不**屏蔽网关侧 `stop` / 解锁。  
...
| 0.2 | 2026-08-06 | 童锁（面板失效、网关仍可控）；登记上+下≈5s 重置待逆向；仲裁优先级 |

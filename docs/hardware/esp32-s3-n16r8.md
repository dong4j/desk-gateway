...
| WS2812 RGB | 由 **GPIO48** 驱动；当前固件未使用。外接红黄蓝灯珠走 GPIO1 / GPIO2 / GPIO8 |
| PWR LED | 上电常亮，不可控 |
...
| 状态 RGB | 1 | GPIO48 板载 WS2812，当前空闲 |
| 外接状态灯 | 3 | GPIO1 红 / GPIO2 黄 / GPIO8 蓝；固件已驱动，真机未验收 |
| 预留 | 若干 | GPIO17 留给功放 `SD` 等 |

N16R8 可用 GPIO 足够；具体引脚表在《硬件设计》冻结，本文只定主控与禁脚。

### 4.3 GPIO 分配（状态灯已冻结）

避开：USB 占用的 19/20（若要用原生 USB）、Strapping 相关脚、35/36/37、以及开发期想保留的 UART0（43/44）。GPIO1 / GPIO2 / GPIO8 已预留给外接状态灯，不要改作他用。

Phase 1 建议优先从下列里挑：

| 候选 | 原因 |
|---|---|
| GPIO1, GPIO2, GPIO8 | **已预留**给红 / 黄 / 蓝状态灯，不要再给别的外设 |
| GPIO4, GPIO5, GPIO6, GPIO7 | 已用于控制盒与原厂面板 |
| GPIO10–GPIO16 | 已用于 ToF / OLED / MAX98357A |
| GPIO41, GPIO42 | 远离 USB，仍可作后续外设 |
...
| 状态指示 | GPIO1/2/8 外接灯珠 | 固件已驱动，真机未验收；GPIO48 WS2812 板载仍空闲 |
...
| 外接状态灯 | GPIO1 红 / GPIO2 黄 / GPIO8 蓝，共阴有源高；固件已驱动，真机未验收 | 2026-08-17 |
...
| 0.2 | 2026-08-17 | 预留 GPIO1/2/8 外接红黄蓝状态灯；GPIO48 板载 RGB 仍空闲 |
| 0.3 | 2026-08-17 | 默认固件驱动 GPIO1/2/8 状态灯；真机亮灯未验收 |

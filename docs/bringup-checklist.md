...
- [x] 下降约 15s 超时自动停（已有 `hold timeout -> stop` 真机日志；上升改由松手、显式 STOP、档位目标或最高安全高度停止）
...
- [ ] 高度未知时按住升保持可用；首个真实高度帧建立限高锚点，不能因等待显示帧自动停止
- [ ] 档位 1 在高度未知时只向安全低位下降，收到高度后继续闭环到 `64 cm`
- [ ] 档位升降期间高度帧间隔超过 `2.5s` 时不中途停止；缺帧时间本身不能触发最高高度限制
- [ ] 上升日志中的反向值（如 `89→87`）被拒绝，不覆盖最后可信高度
- [ ] 到达已确认最低位 `64 cm` 后继续短按下降，仍保留 `64 cm`，上升与档位 4 不被误置灰
- [ ] 已接受高度达到最高安全高度提前停止余量后保留最后主机高度并锁住上升；下降收到安全区新帧后自动解锁
- [ ] 控制盒轮询诊断中 UP/DOWN 的 `key_tx=开始/完成` 基本一致、`abort=0`，上升不再因伪 START/STOP 断续

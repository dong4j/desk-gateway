...
- [x] 下降约 15s 超时自动停（已有 `hold timeout -> stop` 真机日志；上升当前由松手、显式 STOP 或档位目标停止）
- [x] 档位 1（64 cm）/4（102 cm）点击后按真实高度自动升降，到达目标附近自动停止
- [ ] 最高安全高度运动拦截暂时停用；保存值不得禁用按住升、档位 4 或触发 `DR=0x2E`
- [ ] 高度未知时，上升和下降保持可用，档位 4 禁用；任一手动方向收到首帧后自动恢复高度
- [ ] 高度未知时按住升保持可用；首个真实高度帧建立限高锚点，不能因等待显示帧自动停止
- [ ] 档位 1 在高度未知时只向安全低位下降，收到高度后继续闭环到 `64 cm`
- [ ] 档位升降期间高度帧间隔超过 `2.5s` 时不中途停止；缺帧时间不得触发任何手动运动停止
- [ ] 上升日志中的反向值（如 `89→87`）被拒绝，不覆盖最后可信高度
- [ ] 到达已确认最低位 `64 cm` 后继续短按下降，仍保留 `64 cm`，上升与档位 4 不被误置灰
- [ ] 控制盒轮询诊断中 UP/DOWN 均持续读取当前 DR，直到松手或显式 STOP；本轮不以高度数据作为停止条件。`key_tx=开始/完成` 应基本一致、`abort=0`；`tx_guard` 非零表示固件正确屏蔽了延迟到达的发送侧 SDA 中断

...
- [ ] Web、BLE、旋钮分别持续上升时运动流畅，且没有 `controller key-poll gap`；若出现该日志，记录
      `max`、`reads`、`DR` 与可见停顿的对应时间
- [ ] 连续上升期间 `0x24` 单字节事务能自行回到地址等待态；延迟 DAT 边沿不会在地址或数据字节
      中途重置状态机，正常约 `3.7 ms` 键轮询不依赖 STOP 中断送达
...
| 时序风险 | 控制盒约每 `3.7 ms` 读取一次 `0x24`；SCL 边沿约每 `52 us` 到达。NimBLE、Wi-Fi、GPIO ISR 和高度任务原先全部位于 Core 0 |
| 当前修正 | GPIO 软件 I²C ISR 改在 Core 1 安装；digit 事件不在 ISR 退出时立即调度；上升预测周期由 `50 ms` 调整为 `100 ms` |
| 诊断日志 | 运动期间累计轮询缺口达到 `20 ms` 时输出 `controller key-poll gap: max=... reads=... DR=...`；显式停止输出 `stop requested: ...` |

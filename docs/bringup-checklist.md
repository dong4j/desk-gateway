...
- [ ] 写 `11` / `14` 可前往 560 / 870 mm；高度未知或上升受限时安全停止且连接保持正常
...
- [x] 手机端 `npm run typecheck` 与 37 项测试通过
- [x] 手机端通用 iOS 无签名构建通过
...
- [ ] 坐姿约 560 mm、站姿约 870 mm 静置时，显示不会因毫米级噪声持续跳动
- [ ] 任一路超过 1 秒没有有效样本后变为未知，不沿用旧值
- [ ] 从不同起点执行 560 / 870 mm 档位，均在目标 `±5 mm` 范围停止

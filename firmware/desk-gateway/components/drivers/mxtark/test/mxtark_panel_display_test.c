/**
 * @file mxtark_panel_display_test.c
...
static void test_two_digit_height(void)
{
    ...
    assert(mxtark_panel_display_encode_height(752, &frame));
...
{
    mxtark_panel_display_frame_t frame;
    assert(mxtark_panel_display_encode_height(1020, &frame));
...
{
    mxtark_panel_display_frame_t frame;
    assert(mxtark_panel_display_encode_height(1295, &frame));
    assert(frame.height_cm == 130);
    assert(frame.digits[0] == 0x44);
    assert(frame.digits[1] == 0xD6);
    assert(frame.digits[2] == 0x5F);
    assert(!mxtark_panel_display_encode_height(399, &frame));
    assert(!mxtark_panel_display_encode_height(2001, &frame));
    assert(!mxtark_panel_display_encode_height(750, NULL));
}
...
int main(void)
{
    ...
    puts("mxtark_panel_display_test: ok");
    return 0;
}
